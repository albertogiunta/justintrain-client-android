...
import static com.jaus.albertogiunta.justintrain_oraritreni.notification.NotificationService.EXTRA_NOTIFICATION_INDEXOUTOFJOURNEYTOBENOTIFIED;
...
class TrainDetailsPresenter implements TrainDetailsContract.Presenter {
    ...
    public void onNotificationRequested(int position) {
        ...
        newNotif.putExtra(EXTRA_NOTIFICATION_INDEXOUTOFJOURNEYTOBENOTIFIED, getIndexOfTrainFromPosition(position));
        ...
        if (busesIndexList.size() == 1 && busesIndexList.get(0) == -1) {
            ...
            if (solution != null && solution.getChangesList() != null) {
                for (Integer busIndex : busesIndexList) {
                    if (solution.getChangesList().size() == 0) {
                        trainList.add(new Train(solution.getChangesList().get(busIndex)));
                    } else {
